...
sed -i '2a\server 127.127.1.0 iburst' /etc/chrony.conf
...
echo $myIP | sed -i "s?ETCD_NAME=\"default\"?ETCD_NAME=\"controller\"?" /etc/etcd/etcd.conf 
...
echo $myIP | sed -i "s?\#ETCD_INITIAL_CLUSTER=\"default=http://localhost:2380\"?ETCD_INITIAL_CLUSTER=\"controller=http://$myIP:2380\"?" /etc/etcd/etcd.conf
